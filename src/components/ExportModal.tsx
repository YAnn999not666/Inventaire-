      ...
      <div className="bg-white w-full max-w-2xl rounded-t-3xl sm:rounded-2xl shadow-2xl border border-slate-200 overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 shrink-0 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <span className="text-sm font-extrabold text-emerald-700 font-mono bg-emerald-100 px-2.5 py-0.5 rounded-lg border border-emerald-200">
        ...
        <div className="p-4 overflow-y-auto flex-1 min-h-[280px]">
          {activeCategories.length === 0 ? (
            ...
            <div className="w-full h-full min-h-[340px] rounded-2xl overflow-hidden border border-slate-300 shadow-inner bg-slate-200">
              <iframe
                src={pdfPreviewUrl}
            ...
            <div className="space-y-3 bg-slate-900 text-slate-100 p-4 rounded-2xl font-mono text-xs border border-slate-800 shadow-inner">
              <div className="text-emerald-400 font-bold text-sm mb-2 border-b border-slate-800 pb-1 flex justify-between">
              ...
              {activeCategories.map((cat) => (
                <div key={cat.id} className="space-y-1 bg-slate-800/80 p-2.5 rounded-xl border border-slate-700/60">
                  <div className="font-bold text-slate-200 uppercase tracking-wide flex justify-between">
                    <span>📦 [{cat.name}]</span>
                    <span className="text-emerald-400">Subtotal: {cat.catTotal} unit.</span>
                  </div>
                  {cat.activeTiers.map((tier) => (
                    <div key={tier.id} className="pl-3 text-slate-300 flex justify-between">
                      <span>• {formatPrice(tier.price)} FCFA</span>
