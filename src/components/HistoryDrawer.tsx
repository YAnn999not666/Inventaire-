                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-bold text-slate-900 text-sm">{record.formattedDate}</span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <span className="text-xs font-mono font-bold bg-emerald-100 text-emerald-800 px-2.5 py-0.5 rounded-full border border-emerald-200">
                  ...
                  {isExpanded && (
                    <div className="bg-slate-900 text-slate-100 p-3 rounded-xl font-mono text-xs space-y-2 border border-slate-800">
                      {activeCats.length === 0 ? (
                        ...
                        activeCats.map((cat) => (
                          <div key={cat.id} className="space-y-1 bg-slate-800/80 p-2 rounded-lg border border-slate-700/60">
                            <div className="font-bold text-emerald-400 uppercase flex justify-between">
                              <span>[{cat.name}]</span>
                              <span className="text-slate-300 font-normal">Subtotal: {cat.catTotal} u.</span>
                            </div>
                            {cat.activeTiers.map((t) => (
                              <div key={t.id} className="pl-2 text-slate-200 flex justify-between">
                                <span>• {formatPrice(t.price)} FCFA</span>
                  ...
                  {/* Action Buttons: Preview, PDF, Share, Load */}
                  <div className="grid grid-cols-4 gap-1.5 pt-2 border-t border-slate-200">
                    <button
                      onClick={() => handlePreviewPdf(record)}
                      className="py-1.5 px-1.5 bg-slate-800 hover:bg-slate-700 text-white font-semibold text-xs rounded-xl transition-colors flex items-center justify-center space-x-1"
                      title="Aperçu du PDF"
                    >
                    ...
                    <button
                      ...
                      className="py-1.5 px-1.5 bg-white hover:bg-slate-100 text-slate-800 font-semibold text-xs rounded-xl border border-slate-300 transition-colors flex items-center justify-center space-x-1"
                    ...
                    <button
                      ...
                      className="py-1.5 px-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs rounded-xl transition-colors flex items-center justify-center space-x-1"
                    ...
                    <button
                      ...
                      className="py-1.5 px-1.5 bg-emerald-800 hover:bg-emerald-700 text-white font-semibold text-xs rounded-xl transition-colors flex items-center justify-center space-x-1"
